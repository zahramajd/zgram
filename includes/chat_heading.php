...
<span class="label label-info" v-if="!is_friend">Unknown</span>
<br>

<img v-bind:src="profile.avatar" class="img-circle">

<button class="btn btn-default btn-sm">More Message</button>

<span v-if="(profile.type)!='person'">
    <button v-if="is_friend" class="btn btn-warning btn-sm" @click="doLeave(profile._id)">Leave</button>
    <button v-else="" class="btn btn-warning btn-sm" @click="addFriend(profile._id)">Join</button>
</span>

<span v-else="">

        <button v-if="is_friend" class="btn btn-warning btn-sm" @click="unfriend(profile._id)">Unfriend</button>¬
